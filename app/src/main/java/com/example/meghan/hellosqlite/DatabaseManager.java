package com.example.meghan.hellosqlite;
...
public class DatabaseManager {
    ...
    public boolean deleteProduct(long productId) {
        ...
        String where =  "_id = ?"; //no where clause can delete entire database
    ...
    public boolean updateQuantity(String name, int newQuantity) {
        ContentValues updateProduct = new ContentValues(); //Contain the data we want to write for the database
        updateProduct.put(QUANTITY_COL, newQuantity);
        String[] whereArgs = { name };
        String where = NAME_COL + " = ?"; //where clause if important because that way you only update one thing, Not the whole database
        ...
        if ( rowsChanged > 0) { //if something updated return true, if nothing updated return false.
    ...
    public int getQuantityForProduct(String productName) {
        ...
        String[] cols = { QUANTITY_COL }; //column array
        ...
        String selection =  NAME_COL + " = ? "; //Where product name equals question mark, question mark replaced with product name
        String[] selectionArgs = { productName };

        //String and names of productes we would like to get back.
        Cursor cursor = db.query(DB_TABLE, cols, selection, selectionArgs, null, null, null);
        //Cursor only provides you with one row at a time

        if (cursor.getCount() == 1) {  //one row returned if none gets you nothing
            cursor.moveToFirst();
            int quantity = cursor.getInt(0); //get an int from the first column that is returned, have to know first column is an integer column
            cursor.close();
            return quantity;
        }
